...
import {
  wishPreviewPublishKanbanBoardToProductionViewerFromLocalServerV1,
  wishPublishKanbanBoardToProductionViewerFromLocalServerV1,
} from "@/lib/wish-publish-kanban-board-to-production-viewer-from-local-server-v1";
import type { WishKanbanBoard } from "@/lib/wish-kanban-board-domain-types";

/**
 * Publica (ou prévia) o quadro do editor no viewer de produção.
 * Body: `{ board?, preview?: boolean }` — sem board, lê SQLite local.
 */
export async function POST(request: Request) {
  ...
  if (body && typeof body === "object") {
    preview = (body as { preview?: unknown }).preview === true;
    if ("board" in body) {
  ...
  if (preview) {
    const result = await wishPreviewPublishKanbanBoardToProductionViewerFromLocalServerV1({ board });
    if (!result.ok) {
      const status = result.code.startsWith("production_") ? 502 : 400;
