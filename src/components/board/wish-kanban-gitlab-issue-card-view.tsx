...
import { WishKanbanBoardModalGitlabIssueDescriptionMarkdownViewClient } from "@/components/board/wish-kanban-board-modal-gitlab-issue-description-markdown-view-client";
...
type WishKanbanGitlabIssueCardViewProps = {
  ...
  /** Modo visualização: sem arrastar, atualizar, remover, DVITU/GUT. */
  viewOnly?: boolean;
};
...
export function WishKanbanGitlabIssueCardView(props: WishKanbanGitlabIssueCardViewProps) {
  ...
  const descriptionMarkdown = wishGitlabIssueSummaryDescriptionMarkdownFromSnapshotDataV1(snapshot);
  const canOpenDescription = Boolean(snapshot);
  ...
  }, [snapshot, isSmartTaskCard, dvituRequiredLabels, viewOnly]);
  ...
  }, [snapshot, isSmartTaskCard, gutRequiredLabels, viewOnly]);

  return (
    <div
      ref={setNodeRef}
      style={style}
      data-wish-kanban-card-id={props.card.id}
    ...
    >
      <div className="flex items-start justify-between gap-2">
        <button
          ...
          className={[
            "flex-1 text-left",
            mutedBySearch || viewOnly ? "cursor-default" : "cursor-grab active:cursor-grabbing",
          ].join(" ")}
          {...(viewOnly ? {} : attributes)}
          {...(viewOnly ? {} : listeners)}
        >
        ...
        <div className="mt-3 flex flex-wrap items-center justify-between gap-2 border-t border-black/5 pt-3 dark:border-white/5">
          {canOpenDescription ? (
            <button
              ...
              title={descriptionMarkdown ? "Ver descrição da issue" : "Ver detalhes (sem descrição no snapshot)"}
          ...
          {!isSmartTaskKanbanIssueUrl(snapshot.webUrl) ? (
            <a
      ...
      <WishKanbanBoardModalGitlabIssueDescriptionMarkdownViewClient
        open={descriptionModalOpen}
        onClose={() => setDescriptionModalOpen(false)}
        snapshot={snapshot}
        descriptionMarkdown={descriptionMarkdown}
        isSmartTaskCard={isSmartTaskCard}
      />

      {!viewOnly && props.onMergeGitlabSnapshotAfterDvitu && snapshot?.webUrl ? (
      ...
      {!viewOnly && props.onMergeGitlabSnapshotAfterGut && snapshot?.webUrl ? (
